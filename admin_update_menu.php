...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE cafeteria SET categoryid=%s, menu_name=%s, menu_price=%s, menu_status=%s WHERE menu_id=%s",
...
if (isset($_GET['menu_id'])) {
  $colname_Recordset1 = $_GET['menu_id'];
}
mysql_select_db($database_cafeteria_system, $cafeteria_system);
$query_Recordset1 = sprintf("SELECT * FROM cafeteria WHERE menu_id = %s", GetSQLValueString($colname_Recordset1, "text"));
...
<!-- Start Menu -->
    ...
    <div class="container">
      ...
      <form action="<?php echo $editFormAction; ?>" name="form1" method="POST" onSubmit="return ValidationEvent()">
                            ...
                            <div class="form-group">
                            <div align="left"><strong><?php echo $row_Recordset1['categoryid']; ?></strong></div>
                            </div>
                             <div align="left">Menu Photo :</div>
                            <div class="custom-file mb-3">
                            <div align="left"><img src="<?php echo $row_Recordset1['menu_photo']; ?>" height="125px;" width="125px"></div>
                             <br>
                            </div>
                            <!--<div class="custom-file mb-3">
                              <input type="file" value="<?php echo $row_Recordset1['menu_photo']; ?>" class="custom-file-input" id="customFile" name="filename" required>
                              <label class="custom-file-label" for="customFile"><?php echo $row_Recordset1['menu_photo']; ?></label>
                            </div>-->
        <div align="left">Menu Name :</div>
                            <div class="form-group">
                              <input type="text" value="<?php echo htmlentities($row_Recordset1['menu_name'], ENT_COMPAT, ''); ?>" class="form-control" id="menu" placeholder="Text Only" name="menu" pattern="[A-Za-z ]+" required>
                            </div>
                             <div align="left">Menu Price :</div>
                            <div class="form-group">
                              <input type="text" value="<?php echo htmlentities($row_Recordset1['menu_price'], ENT_COMPAT, ''); ?>" class="form-control" id="price" placeholder="RM __.__" name="price" required>
                            </div>
                            <div align="left">Menu Status :</div>
                            <div class="form-group">
                              <!--<input type="number" value="<?php echo htmlentities($row_Recordset1['menu_status'], ENT_COMPAT, ''); ?>" class="form-control" id="menu_status" placeholder="0 [is not available] or 1 [is available]" name="menu_status" min="0" max="1" required>-->
                              <select class="form-control" aria-label="Default select example" id="menu_status" name="menu_status">
                                <option selected><?php echo htmlentities($row_Recordset1['menu_status'], ENT_COMPAT, ''); ?></option>
...
<br>
		...
		<button type="reset" class="btn btn-default">Reset</button>
                            &nbsp;
							<button onClick="location.href='admin_menu.php'" class="btn btn-default">BACK</button>
<input type="hidden" name="MM_update" value="form1">
                            <input type="hidden" name="menu_id" value="<?php echo $row_Recordset1['menu_id']; ?>">
                            <input type="hidden" name="ID" value="<?php echo $row_Recordset1['categoryid']; ?>">
        </form>
      </div>
